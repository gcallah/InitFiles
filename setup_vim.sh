...
echo "Going to copy $INI_HOME/zenburn.vim"
cp $INI_HOME/zenburn.vim $VIM_DIR/colors
...
then
	...
	cd $VIM_DIR/after/ftplugin; ln -s $INI_HOME/html.vim .
	cd $VIM_DIR/after/ftplugin; ln -s $INI_HOME/python.vim .
	cd $VIM_DIR/after/ftplugin; ln -s $INI_HOME/text.vim .
	cd $VIM_DIR/after/ftplugin; ln -s $INI_HOME/cpp.vim .
fi
